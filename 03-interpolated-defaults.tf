locals {
  ...
  aad_group_name = contains(["ptlsbox", "ptl", "demo"], var.environment) ? "DTS Contributors (sub:dts-sharedservices${local.sub_env})" : "DTS Contributors (sub:dts-sharedservices-${local.sub_env})"
