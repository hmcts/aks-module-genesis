locals {
  ...
  aad_group_name = contains(["ptlsbox", "ptl"], var.environment) ? "DTS Contributors (sub:dts-sharedservices${local.sub_env})" : "DTS Contributors (sub:dts-sharedservices-${local.sub_env})" ? "DTS Contributors (sub:dts-cftapps${local.sub_env})" : "DTS Contributors (sub:dts-cftapps-${local.sub_env})"
