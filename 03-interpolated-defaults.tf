locals {
  ...
  aad_group_name = var.environment == "ptl" && length(regexall("*cft*", lower(data.azurerm_subscription.current.display_name))) > 0 ? "DTS Contributors (sub:dts-cftptl-intsvc)" : (var.environment == "ptlsbox" && length(regexall("*cft*", lower(data.azurerm_subscription.current.display_name))) > 0 ? "DTS Contributors (sub:dts-cftsbox-intsvc)" : var.environment == "ptl" && length(regexall("*sharedservices*", lower(data.azurerm_subscription.current.display_name))) > 0 ? "DTS Contributors (sub:dts-sharedservicesptl)" : (var.environment == "ptlsbox" && length(regexall("*sharedservices*", lower(data.azurerm_subscription.current.display_name))) > 0 ? "DTS Contributors (sub:dts-sharedservicesptl-sbox)" : (!contains(["ptl","ptlsbox"], var.environment) && length(regexall("*sharedservices*", lower(data.azurerm_subscription.current.display_name))) > 0 ? "DTS Contributors (sub:dts-sharedservicesptl-sbox)" : "DTS Contributors (sub:dts-cftapps-${local.sub_env})")))
