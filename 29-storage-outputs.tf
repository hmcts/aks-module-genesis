data "null_data_source" "storage_outputs" {
  inputs = {
    ...
    terraform_storage_account_id   = azurerm_storage_account.terraform_storage_account[0].id
    terraform_storage_container_id = azurerm_storage_container.terraform_storage_container[0].id
  }
}
